...
class TestOpt:
    ...
    def test_pishi_lingyao(self, mz, xml_name, expected):
        ...
        res = mz.selNotAuditOptList(1)
        actual = len((res['data']['optRecipeList'][0]['infos']))
        assert actual == expected
        # 断言患者基本信息
        engineid = mz.get_engineid(1)
        patient_data = (mz.get_recipeInfo(engineid, 0))['data']['outpatient']  # 断言待审页面
        assert patient_data['height'] == '160cm'
        assert patient_data['weight'] == '60kg'
        assert patient_data['ccr'] == "90"
        # assert patient_data['bsa'] == '1.3471㎡(计算)'
        assert patient_data['isPregnant'] == str(1)
        assert patient_data['isLactation'] == str(1)
        mz.audit_multi(*[engineid])
        patient_data2 = (mz.get_recipeInfo(engineid, 1))['data']['outpatient']  # 断言已审页面
        assert patient_data2['height'] == '160cm'
        assert patient_data2['weight'] == '60kg'
        assert patient_data2['ccr'] == "90"
        # assert patient_data2['bsa'] == '1.3471㎡(计算)'
        assert patient_data2['isPregnant'] == str(1)
        assert patient_data2['isLactation'] == str(1)
